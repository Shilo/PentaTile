@tool
## The PentaTile atlas contract: a typed Resource that bundles
## (a) a layout strategy (PentaTileLayout subclass), (b) a version
...
@export var layout: PentaTileLayout:
	set(value):
		...
		emit_changed()                                                              # propagates to PentaTileMapLayer._on_contract_changed
@export var variation_seed: int = 0


func _on_layout_changed() -> void:
	# Bubble layout's `changed` up; receiver (PentaTileMapLayer) coalesces via _queue_rebuild.
