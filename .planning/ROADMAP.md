...
| 3. Public-Convention Layouts (Blob47 + Tilesetter) | 5/6 (4 executed + Plan 05 SKIPPED per D-86 = b) | Plan 01 complete (Wave 1 prereqs — 8-Moore single-grid propagation patch D-87 + D-86 gate resolved as **option (b) — Tilesetter layouts deferred to v0.3+**; `TILESETTER_DECISION: b` recorded in STATE.md). Plan 02 complete (TBT design-inspiration audit — 364 lines, AP-1..AP-10 anti-pattern register). Plan 03 complete (D-72/D-73 doc ratification — ROADMAP/REQUIREMENTS retitle + README footnote; ATTRIBUTION.md formally banned via Out-of-Scope rows). Plan 04 complete (PentaTileLayoutBlob47Godot — algorithmic 256→47 collapse + 7×7 BorisTheBrave-canonical packing + collapse/hollow tests; 15/15 suite green). Plan 05 SKIPPED per D-86 = (b) — TilesetterWang15 + TilesetterBlob47 + Tilesetter half of TEMPLATE-02 deferred to v0.3+ backlog (no source files / .pngs / tests created; SUMMARY-only commit). Plan 06 (closeout — will record TBT-01-DEFERRED / TBT-02-DEFERRED / TEMPLATE-02-DEFERRED + comprehensive_bitmask_test extension for Blob47Godot) remaining. | - |
