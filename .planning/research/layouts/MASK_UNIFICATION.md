...
- **LOC budget:** Approach B at v0.2 scope (Tetra/SYMMETRIC + DualGrid16/NON_ROTATING) lands at ~430 LOC total across `penta_tile_map_layer.gd` + `penta_tile_atlas_contract.gd` + `penta_tile_atlas_slot.gd` + new `penta_tile_layout.gd` base + 2 layout subclasses. Wang and Blob47, if added, are ~80-120 LOC each as standalone files. Comfortably under TileMapDual.
- **Migration cost over current Phase 1 plan:** Low. Phase 1 already introduces `PentaTileAtlasContract` as a Resource. The change is to move the `rotation_mode` enum + dispatch logic OFF the contract and INTO a `PentaTileLayout` Resource that the contract REFERENCES. Contract becomes a config bundle; layout becomes the strategy.
...
Reading `addons/penta_tile/penta_tile_map_layer.gd:67-152`, the current pipeline is:
...
Per `.planning/PROJECT.md` and `.planning/research/SUMMARY.md`, the v0.2 milestone scope is **Y-axis variation, top tiles, non-rotating tilesets, per-tile knobs**. The current SUMMARY's Phase 1 introduces `PentaTileAtlasContract` with a `rotation_mode: {SYMMETRIC, NON_ROTATING}` enum + mode dispatch.
...
1. **Code clarity (paint flow):** GOOD initially, BAD long-term. Each `_paint_*` is small and self-contained. The dispatcher is a single `match`. But the dispatcher and the layouts are colocated in `penta_tile_map_layer.gd`; the file balloons as layouts are added. Reading "what happens when I paint a cell?" requires jumping into one of 5 sibling methods.
...
6. **Migration cost:** MEDIUM. Phase 1 plan introduces `PentaTileAtlasContract` with `rotation_mode: {SYMMETRIC, NON_ROTATING}`. To move to Approach A, replace `rotation_mode` with `layout_kind: LayoutKind` on the contract; the existing `_paint_display_cell` becomes `_paint_corner_mask_tetra`; add `_paint_corner_mask_dg16` for what would have been `NON_ROTATING`. Re-uses the contract Resource.

**LOC estimate:** Tetra + DG16 = ~80 LOC of paint methods + ~10 LOC dispatcher inside `penta_tile_map_layer.gd`. With Wang + Blob47 + Subtile added, the layer file grows to ~600-700 LOC just from paint methods. That's larger than v0.1 *just* for layout dispatch.

### Approach B — Polymorphic Layout Resource (Strategy Pattern)

```gdscript
# penta_tile_layout.gd  (NEW base class)
...
func compute_mask(display_cell: Vector2i, has_logic_fn: Callable) -> int:
    push_error("PentaTileLayout subclass must override compute_mask()")
    return 0

func paint(layers: Dictionary, source: int, atlas_coords_provider: Callable,
           display_cell: Vector2i, mask: int) -> void:
    push_error("PentaTileLayout subclass must override paint()")

# Validation hook called by PentaTileMapLayer.update_configuration_warnings()
func validate(contract: PentaTileAtlasContract) -> PackedStringArray:
    return PackedStringArray()
```

```gdscript
# penta_tile_layout_tetra.gd  (the v0.1 behavior)
...
# Contract owns the layout reference: contract.layout: PentaTileLayout
...
1. **Code clarity (paint flow):** EXCELLENT. `penta_tile_map_layer.gd` shrinks (the 16-state match moves out). The paint flow is a 3-line loop calling two virtual methods. Readers can follow "user paints a cell → layout.compute_mask → layout.paint" and then jump into ONE layout file to see the math. Files are split by responsibility, not by call-site.
2. **Extensibility:** EXCELLENT. New layout = new `extends PentaTileLayout` file + register it in the contract's enum (or pick it directly via a typed Resource export). User extension works for free: a user can ship `MyCustomLayout extends PentaTileLayout` in their own project and assign it to their `atlas_contract.layout`.
...
5. **Subtile composition:** FITS NATURALLY. The `paint()` method takes a `layers: Dictionary` and is free to issue any number of `set_cell` calls. A future `PentaTileLayoutSubtileRpgMaker` can issue 4 sub-quad writes per logic cell without changing the dispatcher. This is the primary architectural win over Approach A: subtile composition is *just another layout* that overrides `paint()` to do 4 writes.
6. **Migration cost:** LOW. Phase 1 is currently scoped to introduce `PentaTileAtlasContract`. Approach B requires *adding one more file* (`penta_tile_layout.gd` base + `penta_tile_layout_tetra.gd`) to Phase 1 — about 60 LOC. The contract becomes thinner (it holds a `layout: PentaTileLayout` ref instead of a `rotation_mode` enum). DualGrid16 (the v0.2 NON_ROTATING mode) becomes `PentaTileLayoutDualGrid16 extends PentaTileLayout` — a new file in Phase 3, not a new branch in Phase 1.

**LOC estimate (v0.2 scope = Tetra + DualGrid16):**
- `penta_tile_map_layer.gd`: ~290 LOC (current 260 + ~30 for contract + layers wiring; the 16-state match moves out, recovering ~40 LOC)
- `penta_tile_atlas_contract.gd`: ~50 LOC
- `penta_tile_atlas_slot.gd`: ~30 LOC
- `penta_tile_layout.gd`: ~30 LOC (base class with abstract methods + validation hook)
- `penta_tile_layout_tetra.gd`: ~80 LOC (16-state match + transform tables)
- `penta_tile_layout_dualgrid16.gd`: ~40 LOC (16-entry direct lookup, no transforms)
...
1. **Code clarity (paint flow):** GOOD for blob47 (which actually wants 8 bits); CONFUSING for penta (computes 4 wasted edge checks per cell). Reader sees `universal_mask` and has to understand which bits each layout cares about.
...
2. **The layer file stays small.** Identity guardrail per `PROJECT.md` is "smaller and leaner than TileMapDual." Approach B keeps `penta_tile_map_layer.gd` at ~290 LOC even as layouts proliferate. Approach A grows the layer file linearly with layouts. v0.2 ships 2 layouts; the milestone is "preserve identity," not "ship smallest possible code."
3. **Future-proofing has near-zero v0.2 cost.** Adding the `PentaTileLayout` base + Tetra subclass is ~110 LOC across 2 files. The total Phase 1 LOC is similar to the current plan (the 16-state match moves OUT of the layer file INTO the layout file).
4. **User extensibility comes for free.** Phase 1 doesn't advertise this surface — but a sophisticated user *can* extend `PentaTileLayout` in their own project. That's strictly more value than Approach A offers, at zero implementation cost.
...
Concretely:
- `PentaTileLayoutPenta.paint()`: 1 or 2 writes (primary, optional overlay for masks 6/9)
- `PentaTileLayoutDualGrid16.paint()`: 1 or 2 writes (primary, optional overlay for masks 6/9 — DG16 has the same diagonal-disconnection problem as Tetra)
- `PentaTileLayoutWangEdge.paint()` (future): 1 write
- `PentaTileLayoutBlob47.paint()` (future): 1 write
- `PentaTileLayoutSubtileRpgMaker.paint()` (parking lot): 4 writes per logic cell, to a dedicated subtile layer
...
func _run():
    ...
    var dispatcher: PentaTileLayout = PentaTileLayoutPenta.new()
...
Below is the v0.2 shape for `penta_tile_map_layer.gd::_update_cells()` plus one example layout (`PentaTileLayoutPenta` — preserves v0.1 behavior).

### 5.1 The Generic Layer Pipeline (~25 LOC)

```gdscript
# penta_tile_map_layer.gd  (relevant excerpt)
...
Notes on the shape:
- **`_resolve_layout()`** returns `atlas_contract.layout` if set, otherwise a singleton `PentaTileLayoutPenta` instance configured for the v0.1 hardcoded behavior. This is the "v0.1 fallback" branch from `ARCHITECTURE.md`.
...
func paint(layers: Dictionary, source: int, contract: PentaTileAtlasContract,
...
func _put(layer, source, contract, slot_key, base_transform, cell, pick_alt) -> void:
    var slot: PentaTileAtlasSlot = contract.slot_by_key(slot_key)
...
func validate(contract: PentaTileAtlasContract) -> PackedStringArray:
    ...
    for key in [_FILL, _INNER, _BORDER, _OUTER]:
        if contract.slot_by_key(key) == null:
            warnings.append("PentaTileLayoutPenta: missing required slot '%s'" % key)
    return warnings
```
...
```gdscript
# penta_tile_layout_dualgrid16.gd
...
func paint(layers, source, contract, display_cell, mask, pick_alt) -> void:
    ...
    var slot: PentaTileAtlasSlot = contract.mask_slots[mask]  # 16-entry direct lookup
    ...
    if mask == 6 or mask == 9:
        var complement: PentaTileAtlasSlot = slot.diagonal_complement_slot
        if complement != null:
...
```gdscript
# penta_tile_layout_wang_edge.gd  (FUTURE)
class_name PentaTileLayoutWangEdge
extends PentaTileLayout
...
func paint(layers, source, contract, display_cell, mask, pick_alt) -> void:
    ...
    var slot: PentaTileAtlasSlot = contract.mask_slots[mask]
    if slot != null:
...
```gdscript
# penta_tile_layout_blob47.gd  (FUTURE)
class_name PentaTileLayoutBlob47
extends PentaTileLayout
...
func paint(layers, source, contract, display_cell, mask_index, pick_alt) -> void:
    ...
    var slot: PentaTileAtlasSlot = contract.blob47_slots[mask_index]
    # ... single set_cell ...
```

```gdscript
# penta_tile_layout_subtile_rpgmaker.gd  (PARKING LOT — v0.3+)
class_name PentaTileLayoutSubtileRpgMaker
extends PentaTileLayout
...
**Roadmap action:** Add a one-line entry to `PROJECT.md` Out of Scope section: "RPG Maker A2-A5 subtile composition — Architecturally supported (PentaTileLayout subclass slot reserved); deferred until v0.3+." Document in the layout base class header that `paint()` may issue any number of writes, so future contributors don't constrain it accidentally.
...
- A `class_name PentaTileLayout` registration is the difference between "users can extend" and "users cannot extend." We need `class_name` for the typed `@export var layout: PentaTileLayout` on the contract anyway.
- Costs nothing. We'd get user extension whether we wanted it or not.

**The compromise:**
- `class_name PentaTileLayout` is registered (Approach B requires it).
- `penta_tile_layout.gd` includes a header comment: "EXPERIMENTAL: subclass at your own risk; signatures may change pre-1.0."
...
**Roadmap action:** Add a one-line entry to `penta_tile_layout.gd` header comment when the file is created. No PROJECT.md changes needed.
...
|---|---|---|
| `penta_tile_map_layer.gd` | 290 | v0.1 (260) - inline match (-40) + contract setter (+20) + layout dispatch (+15) + top layer support (+25) + variation hook (+10) |
| `penta_tile_atlas_contract.gd` | 60 | per ARCHITECTURE.md |
| `penta_tile_atlas_slot.gd` | 30 | per ARCHITECTURE.md |
| `penta_tile_layout.gd` (NEW base) | 30 | abstract methods + validate hook |
| `penta_tile_layout_tetra.gd` (NEW) | 70 | 16-state match + slot helpers |
| `penta_tile_layout_dualgrid16.gd` (NEW) | 40 | 16-entry direct + diagonal complement |
| **Total** | **520** | **6 files** |
...
| File | Estimated LOC |
|---|---|
| `penta_tile_map_layer.gd` | 290 + 30 (DG16 paint method) = 320 |
| `penta_tile_atlas_contract.gd` | 60 |
| `penta_tile_atlas_slot.gd` | 30 |
| **Total** | **410** | **3 files** |
...
- `PentaTileAtlasContract` Resource with `rotation_mode: RotationMode {SYMMETRIC, NON_ROTATING}`
- `AtlasSlot` Resource
- `_resolve_slot(mask)` dispatching by `rotation_mode`

The Approach B revision changes Phase 1 to:
- `PentaTileAtlasContract` Resource — drops `rotation_mode`, adds `layout: PentaTileLayout` typed Resource ref + slot config (named slots for Tetra, `mask_slots` array for the layout that wants it)
- `AtlasSlot` Resource — unchanged
- `PentaTileLayout` Resource (NEW base, abstract)
- `PentaTileLayoutPenta` Resource (NEW concrete) — replaces the SYMMETRIC branch of `_resolve_slot`
...
**Phase 3 change:** "Non-rotating mode" becomes "ship `PentaTileLayoutDualGrid16` subclass." Cleaner naming and the user-facing concept is "switch your contract's `layout` to the `DualGrid16` Resource" rather than "set `rotation_mode = NON_ROTATING`."
...
4. **Multi-layer authoring UX.** Phase 1 introduces THREE Resources for what was one enum field (contract + layout + slot). Inspector authoring is more click-y. This is a real downside vs Approach A where users only see "set rotation_mode = NON_ROTATING." Mitigation: ship a default contract `.tres` (already in Phase 5 plan) AND a default `PentaTileLayoutPenta.tres` so users only need to swap layout references, not construct them. Confidence: MEDIUM that the UX cost is acceptable; Phase 1 should validate against actual inspector clicks.
...
- `C:/Programming_Files/Shilocity/PentaTile/.planning/PROJECT.md` — milestone scope, "smaller than TileMapDual" identity guardrail, Out of Scope list.
- `C:/Programming_Files/Shilocity/PentaTile/.planning/research/SUMMARY.md` — current Phase 1 plan, LOC estimates, recommended stack.
- `C:/Programming_Files/Shilocity/PentaTile/.planning/research/ARCHITECTURE.md` — current `_resolve_slot` design, lazy layer pattern, anti-patterns.
- `C:/Programming_Files/Shilocity/PentaTile/.planning/research/PITFALLS.md` — setter discipline, `Resource.changed` storms, alternative_tile bit packing.
- `C:/Programming_Files/Shilocity/PentaTile/addons/penta_tile/penta_tile_map_layer.gd` (260 LOC, lines 67-152 for the current `_paint_display_cell` 16-state match).
...
- `C:/Programming_Files/Shilocity/PentaTile/.planning/research/layouts/TAXONOMY.md` — Researcher 1's output not present. Proceeded with the four mask systems described in the prompt's `<context>` block.

---

*Architecture decision document for: PentaTile v0.2.0 — mask system unification across multiple autotiling layouts*
*Researched: 2026-04-25*
