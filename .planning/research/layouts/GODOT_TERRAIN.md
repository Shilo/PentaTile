...
> **Why this document exists.** PentaTile's pitch is "autotiling that works out of the box if your atlas matches a known layout — no manual peering-bit authoring." That pitch is meaningful only if we can describe, precisely, what the user is currently forced to author *and* what PentaTile will do automatically in its place. This document is the reference spec for the system PentaTile is positioning against.
...
10. [Comparison points for PentaTile](#10-tetratile-comparison)
...
(This is the core difference from PentaTile's design: PentaTile **uses** atlas position as the source of truth — slot 0 is Fill, slot 1 is Inner Corner, etc. — so there is no second layer of metadata to author.)
...
This **could** be re-derived purely from atlas position if the atlas layout is known and follows a canonical tile order — which is exactly the move PentaTile makes for its 4-tile layout, and the move PentaTile would have to extend to support a 47-tile blob layout.
...
- For per-cell-deterministic visual variation (the PentaTile v0.2 milestone goal), the stock terrain system is actively *unsuitable* — its randomness is per-call, not per-cell.
...
**Evidence:** [godot/godot #87929](https://github.com/godotengine/godot/issues/87929) — Click-painting in `MATCH_CORNERS` mode produces a 3×3 block instead of a 2×2 corner. The reporter states *"it seems impossible to create a 2-wide strip of 'match corners' terrain."* Root cause: the algorithm paints **tile centers** rather than **grid intersections**, which is geometrically wrong for corner-based modes. This is structurally identical to the dual-grid problem PentaTile solves — and Godot's solution is to not solve it. This is the strongest argument that **Godot's terrain system cannot replicate dual-grid semantics**, which is why dual-grid addons exist as separate systems.
...
- **TileMapDual** (the project PentaTile compares itself to) — Implements dual-grid autotiling. **Does not use Godot's terrain system at all.** It runs its own dual-grid logic on top of `TileMapLayer.set_cell()`.
...
3. **The metadata model is the *problem*.** Both PentaTile and TileMapDual exist partly to skip peering-bit authoring. Routing through a system whose only input is hand-painted peering bits would defeat the purpose.

**Why PentaTile in particular bypasses it:** PentaTile's mask is the 4-bit corner state of the *display* cell, derived purely from `get_cell_source_id()` reads on the *logic* cell — see `penta_tile_map_layer.gd` lines 155–169. It needs zero per-tile metadata. Routing this through Godot's terrain system would require:
- Synthesizing a terrain-set + terrain on the user's TileSet,
- Synthesizing peering bits for every tile in the atlas (the thing PentaTile's whole pitch is "you don't need to author"),
- Calling `set_cells_terrain_connect` instead of doing the lookup directly,
- Surrendering determinism (PentaTile's mask→tile mapping is a static `match` statement).

This isn't a pure size win for the addon — it's an **identity loss**. The "no peering bits required" guarantee disappears.

---

## 10. Comparison points for PentaTile

### What PentaTile already does that overlaps with Godot's terrain matching

Verified against `addons/penta_tile/penta_tile_map_layer.gd`:

| Capability | Godot terrain | PentaTile current (`_update_cells` → `_paint_display_cell`) |
|------------|---------------|-------------------------------------------------------------|
...
| Multiple-match resolution | Random via `Math::random()` | N/A — PentaTile's mapping is one-to-one (no ambiguity exists) |
...
The overlap is structural: both systems compute a mask from neighbors, then look up a tile. PentaTile is a specialization of the same idea at one specific atlas size with one specific corner-mode mask, achieved by hardcoding the lookup table that Godot tries to learn from peering bits.

### What PentaTile would *gain* by integrating with Godot's terrain system

1. **Stock paint tools "just work."** The TileMap editor's Terrains tab (Connect mode, Path mode, terrain selector dropdown) becomes a PentaTile-aware authoring surface for free. Currently those tools paint PentaTile's logic layer correctly because they paint *tiles*, but they don't speak in terrain semantics — the user can paint any atlas tile, not just "logic-on" or "logic-off."
...
3. **Better dialog with TileMapDual users.** Anyone with a 47-tile blob TileSet already authored for Godot's terrain system could drop PentaTile in if PentaTile speaks the same metadata.
4. **Editor visualization.** The Terrain tab's color overlay would show the user which tiles PentaTile will pick for which mask without a custom inspector plugin.

### What PentaTile would *lose* by integrating

1. **The "no manual bitmask authoring" promise.** Gone. To use Godot's terrain system, every tile needs peering bits painted. Even if PentaTile auto-paints them on first scan, the user now has metadata they can edit and break.
2. **Determinism.** §7 — Godot's matcher is non-deterministic by design. PentaTile's per-cell variation goal (v0.2.0) becomes much harder if tile selection is downstream of `Math::random()`.
...
4. **The 2-wide-strip bug (§8 pain point 3) inherits.** PentaTile's correct behavior on dual-grid corner cases gets clobbered by Godot's tile-center painter.
5. **Lean architecture.** `_update_cells` currently calls `set_cell` directly. Routing through `set_cells_terrain_connect` adds a re-entrant path and forces PentaTile to defend against the matcher repainting the same cells back.

### The verdict for the v0.2.0 redesign

**Don't integrate.** The pain points PentaTile's pitch attacks (manual peering-bit authoring, non-determinism, dual-grid incompatibility) are exactly the features Godot's terrain system *brings*. The integration would invert PentaTile's value proposition.

**What PentaTile should do instead** to address the "stock paint tools" gap:
...
- For the `MATCH_CORNERS_AND_SIDES` 47-tile case (a future roadmap item), the **mask system** (researcher 4's domain) needs a layout descriptor that maps each of the 47 reachable patterns to an atlas slot. That descriptor *is* the equivalent of Godot's per-tile peering bits — but it's **per-layout** (authored once by PentaTile maintainers), not **per-atlas** (authored repeatedly by every user).
- This is the same trick PentaTile already does for the 4-tile layout: bake the table into the addon, expose only "which atlas layout am I" as a config knob.

Restated: **Godot's terrain system asks every user to author the lookup table. PentaTile asks the addon author to author the lookup table once, and the user only declares which table to use.** That is the entire competitive position, and any integration with the engine's terrain system has to preserve it.

### README copy-ready bullet for "Why PentaTile vs Godot's terrain"

> Godot's built-in terrain system asks you to paint up to 8 peering bits onto every tile in your atlas — 376+ clicks for a single-terrain blob set, and the matcher is non-deterministic by design ([discussion #10948](https://github.com/godotengine/godot-proposals/discussions/10948), [proposal #7670](https://github.com/godotengine/godot-proposals/issues/7670)). PentaTile reads atlas position directly: drop a 4-tile atlas in PentaTile's order and you get correct dual-grid autotiling without authoring any per-tile metadata.
...
- §3 "validity matrix" rules per mode + grid shape inferred from source structure. HIGH on the *square grid* validity table; MEDIUM on what changes for hex/iso (out of scope for PentaTile).

---

*Reference document for: PentaTile mask-system unification (researcher 4 input) and README "vs Godot's terrain" copy.*
*Researched: 2026-04-25*
