...
| **Config file** | `addons/penta_tile/tests/run_tests.ps1` (Windows local dev, registers 18 tests) + `addons/penta_tile/tests/run_tests.sh` (Linux CI mirror, NEW — Plan D Wave 1) |
| **Quick run command** | `pwsh -File addons/penta_tile/tests/run_tests.ps1 -NoPause -Test all` (local Windows, ~2 min) |
| **Full suite command** | `bash addons/penta_tile/tests/run_tests.sh` (Linux CI, executed by release workflow step "Run tests") |
...
- **After every task commit:** Run `pwsh -File addons/penta_tile/tests/run_tests.ps1 -NoPause -Test all` (local Windows, 18 tests must stay green).
...
| 5-A-* | A (Demo refresh) | 1 | DEMO-02 | — | All 8 instances have `tile_set = null` and resolve via `get_fallback_tile_set()` | composed-canvas test | `pwsh -File addons/penta_tile/tests/run_tests.ps1 -Test fallback_routing_test -NoPause` | ✅ Phase 4 shipped this test | ⬜ pending |
...
| 5-D-* | D (Release workflow) | 1 | (CI infra) | — | `addons/penta_tile/tests/run_tests.sh` mirrors `run_tests.ps1` semantics on Linux | unit (test runner self-test) | `bash addons/penta_tile/tests/run_tests.sh` (locally on WSL or via workflow dry-run) | ❌ Plan D Wave 1 creates it | ⬜ pending |
...
- [ ] `addons/penta_tile/tests/run_tests.sh` — NEW Linux mirror of `run_tests.ps1`; Plan D Wave 1 ships it. CI cannot execute Phase 4 tests on `ubuntu-latest` until this exists.
