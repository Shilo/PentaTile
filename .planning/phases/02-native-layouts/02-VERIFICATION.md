...
| 6  | AUTO mode maps atlas axis size 1/2/3/4/5 → ONE/TWO/THREE/FOUR/FIVE; 0/6+ → warning | ? HUMAN | `resolve_active_mode()` confirmed in source (lines 219-238); AUTO_STRIP branch confirmed; detection logic correct in code; actual dispatch against a live TileSet requires editor |
| 7  | AUTO_STRIP independently detects each strip's tile count via has_tile() | ✓ VERIFIED | `resolve_strip_modes()` implemented in penta_tile_layout_penta.gd; uses has_tile() per-strip; confirmed in source |
...
| WR-03 | penta_tile_synthesis.gd | ⚠️ Warning | `synthesize_strip` source-coords formula assumes spacing=5; wrong for AUTO_STRIP multi-strip atlases where different strips use different modes |
...
WR-01 and WR-03 are the most relevant to correctness: WR-01 could cause incorrect collision polygons for non-convex source data; WR-03 would produce wrong synthesis for AUTO_STRIP multi-mode atlases. Both are deferred per 02-REVIEW.md.
