...
- **D-68: The 7-wave breakdown from the FOURTH SUPERSESSION (D-67) is locked verbatim; not re-litigated this round.** The size of Phase 2 (~30 reqs, 17 success criteria) was discussed and the user chose to keep it as one phase rather than split into Phase 2.0 / 2.5 — the dependency graph is tight enough that a split would not buy clarity.
  ...
  - **Wave 7 — Closeout.** LOC checkpoint vs Phase 1 baseline (~530 LOC). Expected cumulative range: ~1230-1530 LOC (per the per-file LOC budget in `<code_context>`). Flag for design review only if cumulative materially exceeds ~1500 (the upper bound of expected) — at that point investigate whether `_synthesize_strip` is over-spec'd. Do NOT fire the flag on the lower-half of the expected range (~1230-1400) since that's a clean execution. CHANGELOG entries per DOC-04.
...
- **Phase 5 DOC-03 IsolatedCell-corner-quality callout (post-audit follow-up).** The Path B locked decision (D-69 / 02-02-PLAN.md Gate 1 OuterCorner exception) means slot 0's corners are rotated at render time to produce all 4 OuterCorner orientations across ALL modes (ONE/TWO/THREE/FOUR/FIVE) — not just prototyping ONE mode. The "sacrificing quality for less quantity" framing fits ONE mode well but slightly underplays the cost in FOUR/FIVE where artists put real effort into Border/InnerCorner/OppositeCorners and may not realize OuterCorner quality is gated on slot 0's IsolatedCell silhouette. DOC-03 ("Authoring a Custom Layout") MUST explicitly call this out: "When authoring a `bitmask_template` for `PentaTileLayoutPenta`, design slot 0's corners with care — they are the source of OuterCorner art across every mode, including pure-authored FIVE mode. Best practice: keep the IsolatedCell's center fill + edges visually fade-able (transparent or near-transparent extras) so the rotated rendering reads as a single corner. If pixel-perfect OuterCorner is required, OuterCorner cannot be authored as a separate slot in v0.2 — wait for a future milestone or override `mask_to_atlas` directly." Backlog this against Phase 5 DOC-03.
