...
- Test script: `tests/determinism_test.gd` (extends SceneTree; runs headlessly)
...
- Run command: `Godot_v4.6.2-stable_win64.exe --headless --path . --script tests/determinism_test.gd`
...
BASELINE_HASH from `tests/baselines/four_mode_5x5.txt`: **2986698704**
...
- [x] **PASS** — All three sub-results green: (1) all 11 main hashes identical AND match `BASELINE_HASH` from `tests/baselines/four_mode_5x5.txt`; (2) all 8 transform_vertex rows match expected output verbatim from 02-02-PLAN.md Gate 2 table; (3) all 10 clip_polygon_to_subrect hashes identical.
...
- **Test file committed:** `tests/determinism_test.gd` is committed alongside this report for future re-runs. Phase 5 / future regressions can re-run it headlessly.
