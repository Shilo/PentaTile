...
- `tests/blob_47_collapse_test.gd` (70 LOC) — Pure-math unit test. Asserts 256 collapses are all in dict, dict.size()==47, idempotence on 5 spot values, mask=0/255 dispatch coords.
- `tests/blob_47_hollow_test.gd` (179 LOC) — Composed-canvas integration test. 5×5 hollow ring at (0,0)-(4,4) minus (2,2). Asserts bbox bounds, hole emptiness, PRE-BAKED W-5 strict-equality on bbox/canvas dimensions + painted_count.
...
- `tests/run_tests.ps1` (+2 LOC) — Registered `blob_47_collapse_test` and `blob_47_hollow_test` in `$allTests` (between `determinism_test` and `single_grid_8_moore_propagation_test`).
...
- **Files modified:** `tests/blob_47_hollow_test.gd`
...
- `tests/blob_47_collapse_test.gd` exists. ✓
- `tests/blob_47_hollow_test.gd` exists. ✓
