...
TetraTile is a young, focused addon with minimal scope (v0.1.0). The implementation is clean and well-researched. No critical bugs or security issues detected. Concerns are primarily about architectural limitations, performance unknowns, and future scalability.
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 11-14, 182-185)
- Impact: Users cannot swap out atlases without rebuilding source code. No flexible terrain metadata system.
- Fix approach: Future roadmap includes optional "Y-axis variations" (atlas rows) and "TetraBake" (procedural composition). These are intentionally deferred until the 4-tile contract is proven in production.
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 67-83)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 129-140)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 199-202, 220, 244)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 64, 260)
...
- Safe modification: Ensure `TetraTileMapLayer` is always `add_child()`-ed before accessing visuals. The demo does this correctly.
- Test coverage: No test covers the case where a `TetraTileMapLayer` is created, configured, and used before `_ready()` completes.

**Atlas layout hard-coded in mask calculations:**
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 182-185)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 101-105, 155-165)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 82-83)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (entire file)
...
- Files: `addons/tetra_tile/demo/demo_runtime_painter.gd`, `addons/tetra_tile/tetra_tile_map_layer.gd`
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (performance-sensitive loops)
...
- Limit: Unknown. Likely bottleneck is Godot's `TileMapLayer` performance, not TetraTile's mask logic.
...
- Limit: Likely hits Godot's layer/draw-call limits before TetraTile's code is the bottleneck.
...
- Risk: RESEARCH.md and README both note that Godot may force cleanup when a TileMapLayer is set `visible=false`. TetraTile uses `self_modulate.a` on the *logic layer* to hide it instead.
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (lines 248-251), README.md (line 101)
...
- Files: `addons/tetra_tile/tetra_tile_map_layer.gd` (line 1, line 67)
...
- Risk: None. TetraTile is a pure class with no global dependencies.
...
TetraTile depends only on Godot 4.6's native `TileMapLayer`, `TileSetAtlasSource`, and `Vector2i` APIs. No third-party packages. No version constraints beyond "Godot 4.6+".
...
- Problem: TetraTile only handles a single binary terrain (filled vs empty). No support for grass-to-dirt, water-to-land, or multi-terrain mosaics.
- Blocks: Users cannot create complex level layouts with multiple terrain types in a single map using one TetraTileMapLayer.
- Workaround: Use multiple TetraTileMapLayers, one per terrain type, offset in Z or space.
...
- Roadmap note: Intentionally deferred. TileMapDual supports this with editor integration; TetraTile's scope is intentionally narrow.
...
TetraTile is pure gameplay code with no file I/O, networking, input validation beyond Godot's event handling, or user data handling. No secrets, API keys, or credentials in scope.
